...
def get_config_path(default_path="baseline.yaml"):
    """
    Peek at sys.argv for a `--config=...` (or `--config ...`) flag so we can
    load the YAML before building the main argparse. Any unknown args are
    ignored at this stage; the real parser handles them.
    """
    pre = argparse.ArgumentParser(add_help=False)
    pre.add_argument("--config", type=str, default=default_path)
...
def get_argparse():
    ...
    parser.add_argument('--config', type=str, default='baseline.yaml',
                        help='Path to the YAML config file (default: baseline.yaml)')
    ...
    parser.add_argument('--model_type', type=str, default='ae',
                        choices=['ae', 'domain_cae'],
                        help='High-level model family. "domain_cae" routes to DCASE2023T2-Domain-CAE.')
    parser.add_argument('--score', type=str, default="MSE",
                        choices=["MSE", "MAHALA", "DOMAIN_MAHALA"])
    parser.add_argument('--domain_scoring_mode', type=str, default='weighted',
                        choices=['weighted', 'hard_min'],
                        help='Aggregation rule used by --score DOMAIN_MAHALA.')
    parser.add_argument('--lambda_domain', type=float, default=0.05,
                        help='Weight of the domain BCE loss in the Domain-CAE training objective.')
    parser.add_argument('--domain_prob_clip', type=float, default=0.05,
                        help='Clip p_target into [clip, 1-clip] before weighted DOMAIN_MAHALA '
                             'scoring so the convex combination stays smooth at the extremes. '
                             'Must satisfy 0.0 <= clip < 0.5. Set to 0 to disable.')
