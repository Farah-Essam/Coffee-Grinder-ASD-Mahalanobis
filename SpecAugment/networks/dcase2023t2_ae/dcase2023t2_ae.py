...
class DCASE2023T2AE(BaseModel):
    def __init__(self, args, train, test):
        ...
        if self.args.use_specaugment:
            print(
                "SpecAugment enabled for training only "
                f"(time_mask_param={self.args.specaug_time_mask_param}, "
                f"freq_mask_param={self.args.specaug_freq_mask_param}, "
                f"num_time_masks={self.args.specaug_num_time_masks}, "
                f"num_freq_masks={self.args.specaug_num_freq_masks}, "
                f"prob={self.args.specaug_prob})"
            )
    ...
    def apply_specaugment(self, x):
        ...
        if x.shape[1] != n_mels * frames:
            raise ValueError(
                f"SpecAugment expects flattened input size {n_mels * frames}, "
                f"but got {x.shape[1]}"
            )

        augmented = x.clone()
        if torch.rand(1, device=x.device).item() > self.args.specaug_prob:
            return augmented

        augmented = augmented.reshape(-1, n_mels, frames)
        freq_mask_param = min(self.args.specaug_freq_mask_param, n_mels)
        time_mask_param = min(self.args.specaug_time_mask_param, frames)

        for _ in range(self.args.specaug_num_freq_masks):
            freq_width = torch.randint(0, freq_mask_param + 1, (1,), device=x.device).item()
            if freq_width == 0:
                continue
            freq_start = torch.randint(0, n_mels - freq_width + 1, (1,), device=x.device).item()
            augmented[:, freq_start:freq_start + freq_width, :] = 0

        for _ in range(self.args.specaug_num_time_masks):
            time_width = torch.randint(0, time_mask_param + 1, (1,), device=x.device).item()
            if time_width == 0:
                continue
            time_start = torch.randint(0, frames - time_width + 1, (1,), device=x.device).item()
            augmented[:, :, time_start:time_start + time_width] = 0

        return augmented.reshape(x.shape)
    ...
    def train(self, epoch):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)):
            ...
            input_data = self.apply_specaugment(data) if self.args.use_specaugment and not is_calc_cov else data
